...
- **Implication for the skill:** an audit restricted to disk reports "nothing installed" for exactly the setups carrying the most standing context. Enumerate in-session, and mark those rows as harness-managed: they are turned off in claude.ai / org settings, never by editing files.
- **The usage evidence is weaker for these rows.** `~/.claude/projects` holds CLI and desktop-app transcripts; an account skill used on claude.ai in the browser writes nothing there. On the machine above, all 30 account skills showed zero invocations — which locates the sessions, not the disuse. Treat local zero as "unknown" for managed rows and require corroboration (`/usage` attribution, a duplicate, or the user's own word) before calling one removable.
...
| Slash-only skills (`disable-model-invocation`) | §1.8 |
| Auto-memory off | §1.13 |
